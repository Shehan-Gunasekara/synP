  ...
  const handleVideoUpload = async (
    ...
    if (file) {
      if (!allowedVideoTypes.includes(file.type)) {
        console.error(`Unsupported file type: ${file.type}`);
        setVideoUploadError(
          `Invalid file type. Please upload a video file in one of the following formats: mp4, ogg, wmv, avi, mpeg, mov.`
        );
        return;
      }
      ...
      try {
        const response = await fetch(
          `https://api.cloudinary.com/v1_1/db2kqbmzf/video/upload`, // Replace with your cloud name
          {
            method: "POST",
            body: formData,
          }
  ...
  const generateLipSync = async () => {
    ...
    try {
      ...
      if (audioUrl === "" || audioFile === null) {
        setError("Please upload an audio file to continue.");
        return;
      }
      if (videoUrl === null) {
      ...
      if (!audioDuration) {
        setError("Please upload a valid audio file to continue.");
        return;
      }
      if (audioDuration <= 0) {
        setError("Please upload a valid audio file to continue.");
  ...
  return (
    <Card className="py-6 px-2 sm:p-6 space-y-6 mx-0 sm:mx-14 sm:min-w-[570px]">
      <div className="flex items-center justify-between">
        <h2 className="text-base lg:text-xl font-medium">Lip Sync</h2>
        <p className="sm:text-sm text-xs text-black/60">
          Step 2: Add audio for lip sync
        </p>
      </div>
      ...
      <div className="text-[10px] sm:text-xs  text-gray-500">
        Supported formats: MP3, WAV, M4A, WebM (Max size: 10MB)
      </div>
      {videoUrl ? (
        <>
          {" "}
          <div className="flex items-center justify-between">
            <h2 className="text-base lg:text-xl font-medium">Input Video</h2>
          </div>
          <PreviewCard
            title={""}
            isInput={true}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-row  gap-6 mb-10">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">Enhanced Lipsync</span>
              <div
            ...
            {showInfo && (
              <div
                ...
                onClick={(e) => e.stopPropagation()}
              >
                Enhanced lipsync provides more accurate mouth movements by using
