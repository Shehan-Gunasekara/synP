...
export function GalleryPage() {
  ...
  const { user, userData, userCredits } = useAuth();
  ...
  useEffect(() => {
    const fetchUgcVideos = async () => {
      ...
      try {
        ...
        setError("Failed to fetch UGC videos");
      } finally {
  ...
  const renderContent = () => {
    if (!user) {
      return (
        <div className="col-span-full text-center py-12 space-y-3">
          <p className="text-2xl text-black/70">Please Sign in to the system</p>
        ...
        <Card className="p-12 text-center">
          <div className="max-w-sm mx-auto space-y-4">
            ...
            <p className="text-xs sm:text-sm text-black/50">
              {activeTab === "ugc"
                ? "Start by creating your first AI-powered UGC video"
                : activeTab === "lipsync"
                ? "Add voice to your videos using our lip sync feature"
                : "Start by creating your first Consistent Avatar video"}
            </p>
            <Button
              variant="secondary"
              onClick={() => {
    ...
    return (
      <div className="space-y-8">
        <GalleryGrid videos={currentPageVideos} />
        {activeTab === "ugc" ? (
          <>
            {" "}
            {/* Pagination Controls */}
        ...
        <div className="mb-12 space-y-8">
          ...
          <div className="flex sm:flex-row flex-col justify-center gap-2">
            ...
            >
              ...
              UGC Videos {totalUGCVideos && `(${totalUGCVideos})`}
            </button>
            <button
              type="button" // Ensures button does not submit a form
              className={`inline-flex items-center justify-center rounded-full font-medium transition-all duration-200 focus:outline-none text-[10px] sm:text-sm md:text-sm px-3 sm:px-4 py-2 sm:py-2 ${
            ...
            >
              ...
              Lip Sync {totalLipsncVideos && `(${totalLipsncVideos})`}
            </button>
            <button
              type="button" // Ensures button does not submit a form
              className={`inline-flex items-center justify-center rounded-full font-medium transition-all duration-200 focus:outline-none text-[10px] sm:text-sm md:text-sm px-3 sm:px-4 py-2 sm:py-2 ${
            ...
            >
              <Video className="w-4 h-4 mr-2" />
              Consistent Avatar {totalMinMaxVideos && `(${totalMinMaxVideos})`}
            </button>
          </div>
        </div>{" "}
        {renderContent()}
